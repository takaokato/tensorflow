...
void DepthToSpaceOpFunctor<Device, T, data_format>::operator()(
    const Device& d, typename TTypes<T, 4>::ConstTensor input, int block_size,
    typename TTypes<T, 4>::Tensor output) {
  assert(false);
}
template <typename Device, typename T, TensorFormat data_format>
void DepthToSpaceOpFunctor<Device, T, data_format>::operator()(
    const Device& d, typename TTypes<T, 5>::ConstTensor input, int block_size,
